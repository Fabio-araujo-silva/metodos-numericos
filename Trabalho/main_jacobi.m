...
D = diag(A_mod);        % extrai diagonal
R = A_mod - diag(D);    % parte restante da matriz

% Iterações de Jacobi
for k = 1:max_iter
    x_new = (Lado_direito - R * x) ./ D;
...
% Se não convergir
if k == max_iter
    warning('Método de Jacobi não convergiu após %d iterações.', max_iter);
end
